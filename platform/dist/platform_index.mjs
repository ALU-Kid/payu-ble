...
var POOL_SIZE, _parts, _type, _size, _endings, _a, _Blob, Blob, fetch_blob_default;
...
// platform/platform_gps_helper.ts
var GPSLocationHelper = class {
  constructor(options = {}) {
  ...
  async getIPBasedLocation() {
    try {
      ...
      const response = await fetch2(this.options.ipServiceURL);
      if (!response.ok) {
      ...
      if (data.status === "success" && data.lat && data.lon) {
        this.currentLocation = {
          ...
          accuracy: 1e4,
          // IP location is typically city-level (10km accuracy)
          timestamp: Date.now()
        };
...
function createPlatformHelpers() {
  ...
  return {
    ...
    gpioButton: platformInfo.hasGPIO ? (pin) => gpioButton(pin) : (_pin) => {
    ...
    macOnNetwork: (mac) => macOnNetwork(mac),
    // GPS with automatic fallback
    gpsLocation: (zone) => gpsLocation(zone),
    // Platform info
...
export { GPIOButtonHelper, GPSLocationHelper, NetworkMACHelper, calculateDistanceBetween, createGPIOButtonTrigger, createGPSLocationTrigger, createMACNetworkTrigger, createPlatformHelpers, discoverLocalDevices, getCurrentPosition, getPlatformInfo, gpioButton, gpsLocation, isDeviceOnNetwork, macOnNetwork, updateCoreHelpers };
//# sourceMappingURL=platform_index.mjs.map
//# sourceMappingURL=platform_index.mjs.map